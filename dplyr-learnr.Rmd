---
title: "Penguin wrangling: some basic data wrangling in R with dplyr functions"
...
*R* is a cool programming language - which means it's a way for you to give your computer directions to do specific things. That can be basic calculations, complex models, or even making graphs and tables! 
...
### Keep rows that match string(s)

The `dplyr::filter()` function lets us set conditions for different variables, so that only rows satisfying our conditions are kept. 

Often, we will want to keep rows (observations) that satisfy a condition we set for a *string* (often a description). Some examples would be: 

- Only keep penguins observed on Torgersen Island
- Make a subset containing only chinstrap penguins
- Create a subset containing female gentoo penguins 

When setting conditions to match a character string, make sure to put the string in quotation marks, as in the examples below.

#### `r fa("fas fa-robot", fill = "purple")` Example 1

Make a subset with only chinstrap penguins.

In the code below, we **filter** the **penguins** data to only keep rows where the entry for **species** exactly matches "Chinstrap" (note that it is case sensitive). Press "Run" to see the outcome! 
...
Make a subset with chinstraps and gentoos.
...
Make a subset with Adélie penguins on Dream Island
...
We can create a subset starting from penguins that only contains observations for Adélie penguins on Dream Island as follows:
...
Keep penguins with a flipper length greater than 200 mm
...
Keep penguins with a body mass less than or equal to 2900 g
...
Keep Adélie penguins with a bill length greater than 40 mm

We can use conditions for strings and values in combination, too. In this example, the species should match "Adelie" **and** the bill length (`bill_length_mm`) must be greater than 40 mm.
...
In the code below, the `!=` operator is used within `filter()` to keep observations where the species variable **does not match** "Chinstrap". 
...
Use `filter()` to create a subset from `penguins` that contains observations for female Adélie penguins with bill lengths less than 35 mm. 
...
Within select, list the variables that you want to keep in your new subset, separated by commas. Note: you can also use `select()` to reorder columns - the order they are listed within `select()` is the order they'll appear in the new subset.
...
Select a range of sequential variables using a colon `:` between inclusive endpoint variables. For example, to select from columns `giraffe` to `narwhal` the entire range is referenced with `giraffe:narwhal`.
...
Keep columns from `species` to `bill_depth_mm` *and* `year` from the penguins data.

Note that additional (non-sequential) variables are added after a comma. 
...
Keep columns that start with "bill"

```{r select_8, exercise = TRUE}
penguins %>% 
...
Keep columns that end with "mm"

```{r, select_9, exercise = TRUE}
penguins %>% 
  select(ends_with("mm"))
```
...
Keep columns that contain "length"

```{r select_10, exercise = TRUE}
penguins %>% 
...
```{r select_11, exercise = TRUE}
penguins %>% 
  select(contains("length") | starts_with("bill"))
```

### Examples of select in sequence
...
Move any factor variables (hint: `is.factor`) to after an integer variable (`is.integer`).
...
For example, if I had a data frame `df` with columns `A` and `B`, I can add a new column `C` that is the sum of `A` and `B` as follows:

  `df %>% mutate(C = A + B)` (or use `sum(A,B)`)

...as demonstrated by the helper monsters below:
...
Add a new column to penguins with body mass converted to kilograms.
...
Use `group_by()` and `summarize()` to prepare a summary table containing the mean of penguin body mass, grouped by penguin species. 

Note that the `na.rm = TRUE` argument is added to exclude missing values. 

```{r group_summarize_1, exercise = TRUE}
penguins %>% 
  group_by(species) %>% 
...
```{r group_summarize_2, exercise = TRUE}
penguins %>% 
  group_by(species) %>% 
  summarize(mass_mean = mean(body_mass_g, na.rm = TRUE),
...
```{r group_summarize_3, exercise = TRUE}
penguins %>% 
...
Use `group_by()` and `summarize()` to prepare a summary table containing the maximum and minimum flipper length for male Adélie penguins, grouped by island. 

Here, we'll first use `filter()` to only keep rows for male Adélie penguins, then use `group_by()` and `summarize()` to find the minimum and maximum flipper length **by island**, & put them in a nice table. 

```{r group_summarize_4, exercise = TRUE}
penguins %>% 
...
See more information on `dplyr::across()` [here](https://dplyr.tidyverse.org/reference/across.html).
